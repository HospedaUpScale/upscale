...
function Section({
  ...
  return (
    <section id={id} className="scroll-mt-28 border-b border-slate-100 py-10 first:pt-0 last:border-b-0">
      <h2 className="mb-4 text-xl font-extrabold tracking-tight text-slate-900 md:text-2xl">{title}</h2>
      ...
      <main>
        <section className="border-b border-slate-100 bg-white py-16 md:py-24">
          <div className="mx-auto max-w-4xl px-4 text-center">
            <p className="mb-4 text-xs font-bold uppercase tracking-widest text-[#0229C4]">Proteção de dados</p>
            <h1 className="mb-6 text-4xl font-extrabold leading-[1.1] tracking-tight text-slate-900 md:text-5xl">
              Política de Privacidade e Proteção de Dados
            </h1>
            <p className="mx-auto max-w-2xl text-sm leading-relaxed text-slate-500 md:text-base">
              Esta política explica, em conformidade com a Lei Geral de Proteção de Dados Pessoais (Lei nº
              13.709/2018 — LGPD), como a Upscale coleta, usa, armazena, compartilha e protege os dados pessoais
              tratados em nosso site e em nossos serviços de assessoria em WhatsApp Marketing.
            </p>
            <p className="mt-4 text-xs font-medium text-slate-400">Última atualização: 15 de setembro de 2026</p>
          </div>
        </section>

        <section className="mx-auto max-w-4xl px-4 py-10">
          <div className="rounded-[1.5rem] border border-slate-100 bg-white p-6 shadow-[0_20px_50px_rgba(0,0,0,0.06)] md:p-8">
            <p className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-400">Neste documento</p>
            <nav className="grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
              {sections.map((item) => (
                <a
                  key={item.id}
                  href={`#${item.id}`}
                  className="text-sm text-slate-500 transition-colors hover:text-[#0229C4]"
                >
                  {item.label}
                </a>
              ))}
